...
public class PhisicsButton : MonoBehaviour
{
    ...
    void Start() {
        ...
        if (transform.eulerAngles != Vector3.zero) {
            ...
            upperLowerDiff = buttonLowerLimit.position.x - buttonUpperLimit.position.x;
    ...
    void Update() {
        buttonTop.transform.localPosition = new Vector3(buttonTop.transform.localPosition.x, 0, 0);
        buttonTop.transform.localEulerAngles = new Vector3(0, 0, 0);
        if (buttonTop.localPosition.x <= 0)
            ...
            buttonTopRigid.AddForce(( -1 * buttonTop.transform.right) * force * Time.deltaTime);

        if (buttonTop.localPosition.x >= buttonLowerLimit.localPosition.x)
